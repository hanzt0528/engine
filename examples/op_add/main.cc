...
int main()
{
   std::cout << "conv2d main!"<<std::endl;
    ...
    struct ggml_tensor * input2 = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, 28,28,1,1);
    memcpy(input2->data, digit.data(), ggml_nbytes(input));
    ggml_set_name(input2, "input2");

    struct ggml_tensor * result = ggml_add(ctx0, input,input2 );
